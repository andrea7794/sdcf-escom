...
public class Procesa extends HttpServlet {
    ...
    response.sendRedirect("report.jsp?dep="+depto+"&status="+status+"&n_dep="+name_depto);
  /*File reportFile = new File(application.getRealPath("report1.jasper"));
    ...
    ouputStream.close();*/
          ...
          out.println("<!DOCTYPE html><html xmlns=\"http://www.w3.org/1999/xhtml/\"><head><meta http-equiv=\"Content-Type content=\"text html; charset=utf-8\"/>"
           ...
           + "<h2>actual"+new java.io.File(".").getCanonicalPath() +"</h2>"
           + "<%Connection conn=null; try { Class.forName(\"com.mysql.jdbc.Driver\"); //se carga el driver"
           + "conn= DriverManager.getConnection(\"jdbc:mysql://localhost:3306/banco\",\"root\",\"060590\");"
           + "out.print(\"conexion CTM! xD\");} catch (Exception ex) { ex.printStackTrace(); } "
